...
if paraNoOfTest == 1 :
    ...
    file1 = "200530-1-收到汇总.xlsx"
    file2 = "/Users/benjamin/同步文件/_BENJAMIN/STEMA_Analysis/200530-2-整理去除项.xlsx"
    file3 = "/Users/benjamin/同步文件/_BENJAMIN/STEMA_Analysis/200530-3-分数评判.xlsx"
    file4 = "/Users/benjamin/同步文件/_BENJAMIN/STEMA_Analysis/200530-4-发布成绩.xlsx"
    file5 = "/Users/benjamin/同步文件/_BENJAMIN/STEMA_Analysis/200530-5-分析基础.xlsx"

# 时间偏移设置
# 正态分布最高1%的分数，包含此时间偏移调整，即最高分数在350的基础上加时间偏移
paraTime = (paraNoOfTest - 1) * 2

# 预留给未来按难度系数调整
# 正态分布最高1%的分数，包含此时间偏移调整，即最高分数在350的基础上加难度系数调整
paraDifficulty = 0
...
def MarkingChoice() :
    ...
    # 使用纸质试卷答题，需要本程序判分，累加分数
    if paraNoOfTest==1 or paraNoOfTest==2 :
        # 读取答案表
    ...
    # 使用赛码网的考试系统，自动评判选择题成绩，累加分数
    elif paraNoOfTest == 3:
        for j in range(0, nTotal):
            npNewScore[j] = dfFinal.iloc[j]['选择总分']
        dfFinal.insert(0, '新计算选择总成绩', npNewScore)
...
dfPublish = dfPublish.sort_values(by=['组别', '级别', '省份', '总成绩'], ascending=False)
...
# 写入文件。第1/2次考试本程序评判选择题，有正确答案页。第3次考试用赛码网系统，没有正确答案页。
